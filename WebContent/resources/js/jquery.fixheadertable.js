	...
	$.fn.fixheadertable = function(options) {
		...
		function util_parseDate (format, date) {
			...
			if(date && date !== null && date !== undefined){
				...
				var dfmt = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec",resourceJSON.MonthJanuary,
				                                                                                                resourceJSON.MonthFebruary,
				                                                                                                resourceJSON.MonthMarch,
				                                                                                                resourceJSON.MonthApril,
				                                                                                                resourceJSON.MonthMay,
				                                                                                                resourceJSON.MonthJune,
				                                                                                                resourceJSON.MonthJuly,
				                                                                                                resourceJSON.MonthAugust,
				                                                                                                resourceJSON.MonthSeptember,
				                                                                                                resourceJSON.MonthOctober,
				                                                                                                resourceJSON.MonthNovember,
				                                                                                                resourceJSON.MonthDecember ];
		...
		return this.each(function() {
			...
			function buildTop (table) {
			    ...
			    _fillScrollbar = $('<div class="headtable net-state-default" style="margin-right : 0px" id="divMainFrame"></div>');
				...
				if(options.zebra){
					...
					for (var i = 0; i < objectArray.length; i++){
						...
						for (var j in objectArray[i]){
							...
							tr.append($('<td class="net-widget-content"></td>').html(objectArray[i][j]));
						}	
			...
			function buildHeaders(table) {
				...
				tab_headers.addClass('net-widget-content net-state-default');
				...
				if(options.sortable){
					...
					tab_headers.each(function(i){
						...
						$(this).contents().wrapAll('<div class="net-sort"></div>');
						
						th_div_sort = $('div.net-sort', this);
						
						th_div_sort.click(function(){
							
							tab_headers.removeClass('net-state-hover');
							
							$(this).parent().removeClass('net-state-active').addClass('net-state-hover');
							
							$('span.net-icon', tab_headers).remove();
							...
							if($(this).hasClass('sortedUp')){
								...
								$(this).append('<span style="display : inline-block; vertical-align : middle" class="net-icon net-icon-triangle-1-s"></span>');
								...
								$(this).append('<span style="display : inline-block; vertical-align : middle" class="net-icon net-icon-triangle-1-n"></span>');
							}
					...
					$('div.net-sort', tab_headers).addClass('hover');
				}
				...
				if(options.resizeCol && (options.colratio.length == nbcol)){
					...
					tab_headers.each(function(i){
						...
						var resizer = $('<span class="net-resize"></span>');
					...
					function getOffset(col){
						...
						var ret = 0, cell = $('col', _colgroup), handler = $('th > span.net-resize', _headers)[col], bso = _body[0].scrollLeft;
			...
			function buildBody (table) {
				...
				_body = $('<div class="body net-widget-content"></div>').insertBefore(table).append(table);
				...
				$('td', table).addClass('net-widget-content');
			...
			function buildNavButton(className, callbackClick, buttonClass) {
				...
				var button = $('<div class="button net-state-default ' + buttonClass + '"><span class="net-icon ' + className + '">&nbsp;</span></div>');
				
				_pager.append(button);
				
				button.mouseover(function(){
						
					$(this).addClass('net-state-hover');
						
				}).mousedown(function(){
						
					$(this).addClass('net-state-active');
						
				}).mouseup(function(){
						
					$(this).removeClass('net-state-active');
						
				}).mouseout(function(){
					
					$(this).removeClass('net-state-active');
					
					$(this).removeClass('net-state-hover');
			...
			function buildPager(table) {
			   				...
			   				_pager = $('<div class="pager net-widget-header net-corner-bottom net-widget-content"></div>');
			
				_main_wrapper.append(_pager);
				
				buildNavButton('net-icon-arrowthickstop-1-e', function(){
				...
				buildNavButton('net-icon net-icon-arrowthick-1-e', function(){
					
					moveToNextPage(table);
				}, 'noborder');
				
				buildNavButton('net-icon net-icon-arrowthick-1-w', function(){
					
					moveToPreviousPage(table);
				}, 'noborder');
				
				buildNavButton('net-icon-arrowthickstop-1-w', function(){
					
					_currentpage = 1;
					
					moveToPage(table);
				}, 'net-corner-left noborder');
			...
			_wrapper = $('<div/>').addClass('t_fixed_header net-state-default default ' + options.theme).insertBefore(this).append(this);
			
			_wrapper.css('border', 'none').css('font-weight', 'normal');

			// table corner rounded
			_main_wrapper = $('<div id="divMainFrame" class="t_fixed_header_main_wrapper net-widget net-widget-header net-corner-top ' + options.theme + '"></div>');
			...
			if(options.wrapper){
				...
				var tampon = _wrapper.wrap('<div class="net-widget net-widget-content net-corner-all" style="padding : 5px; font-size : 1em;"></div>').parent();
			...
			if (options.caption != '') {
				...
				var caption = $('<div class="t_fixed_header_caption net-widget-header net-corner-top">' + options.caption + '</div>');
				
				_main_wrapper.prepend(caption).addClass('net-corner-all');
				
				if (options.showhide) {
				
					var showhide = $('<div style="cursor : pointer; display : inline-block; vertical-align : middle; background : none; border : none;" class="net-state-active"><span class="net-icon net-icon-triangle-1-n">&nbsp;</span></div>');
					...
					showhide.click(function(){
						...
						$('span', showhide).toggleClass('net-icon-triangle-1-s');
					});
			...
			if(options.sortable && !isNaN(parseInt(options.sortedColId))) {
				...
				$(cur_th).addClass('net-state-hover')
				
				$('div.net-sort', cur_th).click();
			}
			
			if(options.resizeCol && (options.colratio.length == nbcol)){
			
				_resizeGhost = $('<div class="net-resize-ghost net-widget-header" style="height : ' + _main_wrapper.parent().height() + 'px"></div>');
